function renderShell(activePage, userRole) {
  ...
  const byRole={
    PP:       [{href:'/application_form',icon:'➕',label:'New Application',key:'new_app'}],
    Scrutiny: [{href:'/scrutiny',icon:'🔍',label:'Scrutiny Queue',key:'scrutiny'}],
    MoM:      [{href:'/mom',icon:'📋',label:'MoM Dashboard',key:'mom'},
               {href:'/meetings',icon:'📅',label:'Meetings',key:'meetings'}],
    Admin:    [{href:'/admin',icon:'⚙️',label:'Admin Panel',key:'admin'},
  ...
  const chatbotHtml=`
    <button class="chatbot-fab" id="chatbot-fab" onclick="toggleChatbot()" title="AI Assistant">💬</button>
    <div class="chatbot-window" id="chatbot-window">
      <div class="chatbot-header">
        <span class="chatbot-header-title">🤖 PARIVESH AI Assistant</span>
        <button class="chatbot-close" onclick="toggleChatbot()">×</button>
      </div>
      <div class="chatbot-messages" id="chat-messages">
        <div class="chat-msg bot">👋 Hi! I'm your PARIVESH assistant. Ask me about document requirements, application process, EDS, sectors, or any part of the environmental clearance workflow!</div>
      </div>
      <div class="chatbot-input">
        <input id="chat-input" placeholder="Ask a question…" autocomplete="off">
        <button class="chatbot-send" onclick="sendChatMsg()">➤</button>
      </div>
    </div>`;
