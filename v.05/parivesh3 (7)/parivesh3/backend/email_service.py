...
def notify_payment_success(to, name, app_id, project_name, amount,
                            transaction_id, payment_method, paid_at=''):
    subj = f"[PARIVESH] Payment Confirmed — ₹{amount:,} — {app_id}"
    paid_at_str = str(paid_at)[:16].replace('T', ' ') if paid_at else 'Just now'
    body = f"""<p>Dear {name},</p>
    <p>Your processing fee payment has been <b>successfully received</b> by PARIVESH Portal.</p>
    <div class="box" style="border-color:#2E7D32;background:#F1F8E9;">
      <div class="row"><b>Application ID:</b> {app_id}</div>
      <div class="row"><b>Project Name:</b> {project_name}</div>
      <div class="row"><b>Amount Paid:</b> <span style="color:#2E7D32;font-weight:700">₹{amount:,}</span></div>
      <div class="row"><b>Payment Method:</b> {payment_method}</div>
      <div class="row"><b>Transaction ID:</b> <span style="font-family:monospace">{transaction_id}</span></div>
      <div class="row"><b>Date & Time:</b> {paid_at_str}</div>
      <div class="row"><b>Status:</b> <span style="color:#2E7D32;font-weight:700">✅ SUCCESS</span></div>
    </div>
    <p>Please save this email as your payment receipt. Your application will now proceed for review.</p>
    {_case_btn(app_id)}"""
    _queue(to, subj, _wrap(subj, body))
...
def notify_complaint_received(to, admin_name, complaint_id, user_name,
                               user_email, subject, description, app_id=''):
    subj = f"[PARIVESH] New Complaint #{complaint_id} — {subject}"
    app_row = f'<div class="row"><b>Application ID:</b> {app_id}</div>' if app_id else ''
    body = f"""<p>Dear {admin_name},</p>
    <p>A new complaint has been submitted by a Project Proponent on PARIVESH Portal.</p>
    <div class="box" style="border-color:#E65100;background:#FFF8F0;">
      <div class="row"><b>Complaint ID:</b> #{complaint_id}</div>
      <div class="row"><b>Submitted By:</b> {user_name} ({user_email})</div>
      <div class="row"><b>Subject:</b> {subject}</div>
      {app_row}
    </div>
    <p><b>Description:</b></p>
    <div class="box" style="border-color:#888;background:#f9f9f9;font-style:italic">{description}</div>
    <p>Please log in to the Admin Panel to review and respond.</p>
    <a href="{APP_URL}/admin_complaints" class="btn">→ Open Complaints Dashboard</a>"""
    _queue(to, subj, _wrap(subj, body))


def notify_complaint_response(to, name, complaint_id, subject,
                               response, status, app_id=''):
    subj = f"[PARIVESH] Complaint #{complaint_id} — Admin Response"
    status_color = {'Resolved': '#2E7D32', 'Closed': '#555', 'Open': '#E65100'}.get(status, '#333')
    app_row = f'<div class="row"><b>Application ID:</b> {app_id}</div>' if app_id else ''
    body = f"""<p>Dear {name},</p>
    <p>The Admin has responded to your complaint on PARIVESH Portal.</p>
    <div class="box">
      <div class="row"><b>Complaint ID:</b> #{complaint_id}</div>
      <div class="row"><b>Subject:</b> {subject}</div>
      <div class="row"><b>New Status:</b>
        <span style="color:{status_color};font-weight:700">{status}</span></div>
      {app_row}
    </div>
    <p><b>Admin Response:</b></p>
    <div class="box" style="border-color:#2E7D32;background:#F1F8E9">{response}</div>
    <p>Log in to the portal to view the full complaint thread.</p>
    <a href="{APP_URL}/complaints" class="btn">→ View My Complaints</a>"""
    _queue(to, subj, _wrap(subj, body))
