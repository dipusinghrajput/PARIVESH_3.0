...
CHATBOT_KB = {
    ...
    'eds':        'EDS (Environmental Data Shortfall) is raised when documents are incomplete. You will get an email + notification, then must upload the missing documents and resubmit.',
    'status':     'Track your application status on the Dashboard or Case File page. Every action is logged in the timeline with timestamps.',
    'submit':     'To submit: 1) Create application 2) Upload sector-specific documents 3) Upload affidavit 4) Click Submit. AI will pre-screen your application instantly.',
    'ai':         'AI Pre-Screening automatically checks for missing documents, invalid file types, incomplete project details, and missing EMP/fee documents. Score is 0-100%.',
    'meeting':    'Applications are referred to the Expert Appraisal Committee (EAC). The MoM team schedules a meeting and generates Minutes of Meeting (MoM) after the discussion.',
    'category':   'Category A projects are appraised at the Central level. Category B1 and B2 are state-level. Category is based on project type and scale as per EIA Notification 2006.',
    'sector':     'Supported sectors: Sand Mining, Limestone Mining, Brick Kiln, Infrastructure, Industry, Energy, Coastal, Other. Each has a predefined mandatory document checklist.',
    'email':      'You will receive email notifications for: AI pre-screen results, EDS raised, application referred to EAC, meeting scheduled, MoM generated, and final decision.',
    'unassign':   'Scrutiny officers can be unassigned from an application by the Admin or officer themselves. MoM officers can also be unassigned. The PP is notified by email.',
    'download':   'All uploaded documents are securely stored. Login and open the Case File page to download any document version.',
    'timeline':   'The Case File page shows a complete timeline of every action: AI screening, EDS, document uploads, meetings, and decisions — with timestamps and actor names.',
}
...
def chatbot():
    ...
    if not msg: return jsonify({'reply':'Please type your question.'})
    for keyword, reply in CHATBOT_KB.items():
        if keyword in msg:
            return jsonify({'reply': reply})
    return jsonify({'reply': (
        'I can help with: document requirements, affidavit upload, EDS process, '
        'application submission, AI screening, meeting process, email notifications, '
        'officer assignment/unassignment, sector checklists, and application tracking. '
