...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS applications (
        ...
        mom_officer_id   INTEGER REFERENCES users(id)
    );
    ...
    CREATE TABLE IF NOT EXISTS complaints (
        id             INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id        INTEGER NOT NULL REFERENCES users(id),
        application_id TEXT DEFAULT '' REFERENCES applications(id),
...
# ── USERS ────────────────────────────────────────────────────────────────────
def create_user(name, email, password_hash, role, department='', phone=''):
    conn = get_conn()
    conn.execute("INSERT INTO users (name,email,password_hash,role,department,phone) VALUES (?,?,?,?,?,?)",
...
def update_user_role(uid, role, department, phone=''):
    conn = get_conn()
    conn.execute("UPDATE users SET role=?,department=?,phone=? WHERE id=?", (role, department, phone, uid))
...
def get_application(app_id):
    ...
    row = conn.execute("""
        SELECT a.*,
               u1.name AS creator_name, u1.email AS creator_email, u1.phone AS creator_phone,
               u2.name AS officer_name, u2.department AS officer_dept,
               u2.email AS officer_email, u2.phone AS officer_phone,
               u3.name AS mom_officer_name, u3.department AS mom_officer_dept,
               u3.email AS mom_officer_email, u3.phone AS mom_officer_phone
        FROM applications a
        LEFT JOIN users u1 ON a.created_by=u1.id
        LEFT JOIN users u2 ON a.assigned_officer=u2.id
        LEFT JOIN users u3 ON a.mom_officer_id=u3.id
...
def list_applications(user_id=None, role=None, status=None, sector=None, search=None):
    ...
    sql = """SELECT a.*, u1.name AS creator_name,
             ...
             LEFT JOIN users u1 ON a.created_by=u1.id
             LEFT JOIN users u2 ON a.assigned_officer=u2.id
             LEFT JOIN users u3 ON a.mom_officer_id=u3.id WHERE 1=1"""
    params = []
...
# ── PAYMENTS ──────────────────────────────────────────────────────────────────
def create_payment(application_id, user_id, amount, payment_method, transaction_id):
    conn = get_conn()
    conn.execute("""INSERT INTO payments
        (application_id,user_id,amount,payment_method,transaction_id,status)
        VALUES (?,?,?,?,?,'PENDING')""",
        (application_id, user_id, amount, payment_method, transaction_id))
    conn.commit(); conn.close()
    return get_payment_by_txn(transaction_id)
...
def get_payment_by_txn(transaction_id):
    conn = get_conn()
    row = conn.execute("""
...
def get_payment_by_app(application_id):
    conn = get_conn()
    row = conn.execute("""
        SELECT p.*, u.name AS user_name, u.email AS user_email,
               a.project_name
        ...
        LEFT JOIN users u ON p.user_id=u.id
        LEFT JOIN applications a ON p.application_id=a.id
        WHERE p.application_id=? ORDER BY p.created_at DESC LIMIT 1""", (application_id,)).fetchone()
    conn.close(); return row_to_dict(row)

def get_all_payments():
    conn = get_conn()
    rows = conn.execute("""
        SELECT p.*, u.name AS user_name, a.project_name
        FROM payments p
        LEFT JOIN users u ON p.user_id=u.id
        LEFT JOIN applications a ON p.application_id=a.id
...
# ── COMPLAINTS ────────────────────────────────────────────────────────────────
def create_complaint(user_id, subject, description, application_id=''):
    conn = get_conn()
    # Use NULL (not empty string) so FK constraint passes when no app_id
    app_id_val = application_id if application_id else None
    cur = conn.execute("""INSERT INTO complaints
        (user_id,application_id,subject,description,status)
        VALUES (?,?,?,?,'Open')""",
        (user_id, app_id_val, subject, description))
    cid = cur.lastrowid; conn.commit(); conn.close()
    return get_complaint(cid)
...
def get_complaints_by_user(user_id):
    ...
    rows = conn.execute("""
        ...
        LEFT JOIN applications a ON c.application_id=a.id
        LEFT JOIN users r ON c.responded_by=r.id
        WHERE c.user_id=? ORDER BY c.created_at DESC""", (user_id,)).fetchall()
    conn.close(); return rows_to_list(rows)
...
def respond_to_complaint(cid, admin_id, response_text, status='Resolved'):
    conn = get_conn()
    conn.execute("""UPDATE complaints
        SET admin_response=?, status=?, responded_by=?,
            responded_at=datetime('now')
        WHERE id=?""",
        (response_text, status, admin_id, cid))
    conn.commit()
    conn.close()   # ← close BEFORE querying again
    return get_complaint(cid)
